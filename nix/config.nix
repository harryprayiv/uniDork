{ ... }:
{
  ...
  rename = {
    ...
    # Pure token format: show folder / season folder / file.
    # Three segments, not two — the TV renamer splits on the LAST TWO slashes,
    # not the last one like movies. The old FileBot-style {'Season '+s} is gone;
    # parseFormat can't evaluate expressions and would have silently produced
    # episodes in the show root.
